...
class Service extends GenericService
{
	...
	protected $core = [
		...
		[
			'GET' => [
				'keys'  => [
					'entity_type', 'filter', 'sort', 'field', 'embed',
				],
				'rules' => [
	...
	public function create($data)
	{
		...
		// Create the newly created entity's Identifier.
	...
	public function delete($data)
	{
		...
		// Perform delete.
		$deletePayload = $this->aggregate[$entityContext]->delete($identifierPayload->getData()->pluck('entity_id')->toArray());
	...
	public function purge($data)
	{
		...
		$entityPurgePayload = $this->aggregate[$entityContext]->purge($identifierPayload->getData()->pluck('entity_id')->toArray());
	 ...
	 *
	 * @param  mixed  $key - string || array || object
	 * @return Payload
	 */
	public function getOneByKey($key)
	{
		...
		switch(gettype($key))
		{
			...
			default:
				return new Payload($key, 'invalid_key_type');
		}
	}
			...
			case 'object':
				switch($entity->getType(true))
				{
					...
					default:
						...
						if(array_key_exists($entity->getType(true), $this->getAggregate()))
						{
							// Update the Entity itself.
	...
	/**
	 * Update an Entity by given Entity.
	 *
	 * @param  array  $data
	 * @param  Domain\Entity  $entity
	 * @return Payload
	 */
	...
	/**
	 * Update an Entity by given EntityIdentifier.
	 *
	 * @param  Domain\EntityIdentifier  $entityIdentifier
	 * @param  array  $data
	 * @return Payload
	 */
	 ...
	 * @return Payload
	 */
	protected function getOneByIdentifier($identifier)
	{
	...
	{
		// @todo: May want to limit this method to only use an object for parameter.
		$entityIdentifierContext = IdentifierServiceProvider::getProviderKey();
		...
		switch(gettype($data))
		{
			// Use an Object to update.
			case 'object':
				// Get the base class name of this entity.
				$createData = ['entity_type' => $data->getType(), 'entity_id' => $data->getKey()];

				// Build identifier context.
		...
		return $this->aggregate[$entityIdentifierContext]->create($createData);
	}
	...
	{
		// @todo: May want to limit this method to only use an object for parameter.
		$entityIdentifierContext = ReferenceServiceProvider::getProviderKey();

		// @todo: Will there be any other cases?
		// Get the data ready for creation.
		switch(gettype($data))
		{
			default:
